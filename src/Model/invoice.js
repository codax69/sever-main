  ...
  if (this.isNew && !this.invoiceNumber) {
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        const exists = await this.constructor.exists({ invoiceNumber: candidateNumber });
...
invoiceSchema.methods.calculateTotals = function () {
  ...
  this.pricing.totalAmount = 
    this.pricing.subtotal - this.pricing.couponDiscount + this.pricing.deliveryCharges;
...
invoiceSchema.statics.generateInvoiceNumber = async function () {
  ...
  const nextSequence = result.length > 0 && result[0].maxSequence 
    ? result[0].maxSequence + 1 
    : 1;
